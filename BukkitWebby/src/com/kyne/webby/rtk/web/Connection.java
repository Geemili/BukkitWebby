	...
	public void run() {
		...
		try {
			...
			if(url == null) {
				...
				isAllowedRessource = this.isRestrictedUrl(url) || this.webServer.isAllowedRessource(url) || this.isPredefinedUrl(url);
			}
			...
			// Restricted HTML Pages
				...
				if(this.isUserLoggedIn(clientSocket.getInetAddress())) {
					...
					if("/".equals(url) || "/index".equals(url)) { // Index
						...
						this.rtkModule.backupServer(false);
						this.webServer.printJSONObject(new JSONObject(), clientSocket);
						return;
						...
						this.rtkModule.backupServer(true);
